...
export class Trade {
  ...
  public static openAssetSelectorFor = (side: TradingSide) => {
    if (side === TradingSide.BUY) {
      cy.get(tid('buying-token', tid('balance'))).click()
    } else {
      cy.get(tid('selling-token', tid('balance'))).click()
    }
  }
  ...
  public expectPriceImpact = (priceImpact: string | RegExp, aboveThreshold: boolean) => {
    cy.get(tid('trade-price-impact', tid('value'))).contains(`${priceImpact}`)
  ...
  public expectReceiveToken = (token: string | RegExp) => {
    cy.get(tid('buying-token', tid('currency'))).contains(`${token}`)
  }

  public expectToReceive = (amount: string | RegExp) => {
    cy.get(tid('buying-token', tid('amount'))).should('have.value', `${amount}`)
  }

  public expectPayToken = (token: string) => {
    cy.get(tid('selling-token', tid('currency'))).contains(`${token}`)
  }

  public expectToPay = (amount: string | RegExp) => {
    cy.get(tid('selling-token', tid('amount'))).should('have.value', `${amount}`)
  }

  public sell = (token: string = '') => {
    if (token) {
      cy.get(tid('selling-token', tid('balance')), timeout(2000)).click()

      cy.get(tid(token.toLowerCase()), timeout(2000)).click()
    }
  ...
  public buy = (token: string = '') => {
    if (token) {
      cy.get(tid('buying-token', tid('balance')), timeout(2000)).click()
