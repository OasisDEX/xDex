...
export const cancelOffer: TransactionDef<CancelData> = {
  call: (_data: CancelData, context: NetworkConfig) => context.otc.contract.methods['cancel(uint256)'],
  prepareArgs: ({ offerId }: CancelData) => [offerId.toFixed()],
  options: () => ({ gas: 500000 }),
...
export const offerMakeDirect: TransactionDef<OfferMakeDirectData> = {
  call: ({ kind }: OfferMakeDirectData, context: NetworkConfig) =>
    kind === OfferType.buy
  ...
  prepareArgs: ({ baseAmount, baseToken, quoteAmount, quoteToken }: OfferMakeDirectData, context: NetworkConfig) => [
