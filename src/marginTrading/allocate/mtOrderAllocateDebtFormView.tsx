...
import { AllocateChangeKind, EditableDebt, Message, MessageKind, MTAllocateState } from './mtOrderAllocateDebtForm';
import * as styles from './mtOrderAllocateDebtFormView.scss';

export type CreateMTAllocateForm$ = (proxy: any, request: AllocationRequestPilot) => Observable<MTAllocateState>;
      ...
      return (
        ...
        >
          <Panel style={{ width: '454px' }} className={styles.modalChild}>
            ...
            <PanelBody scrollable={true} paddingVertical={true} style={{ height: '460px' }}>
              {this.props.status === 'loaded' ? (
                this.props.value && this.props.value.progress ? (
                  <ProgressForm {...this.formProps()} />
                ) : (
                  <AllocateForm {...this.formProps()} />
                )
              ) : (
                <LoadingIndicator />
              )}
            ...
            <PanelFooter className={styles.buttons}>
              <Button size="md" onClick={this.props.close} className={styles.button}>
                Close
              </Button>
              {this.props.status === 'loaded' &&
                (this.props.value && this.props.value.progress ? (
...
const DevInfos = ({ value }: { value: MTAllocateState }) => (
  <div style={{ position: 'absolute', top: '1em', right: '1em' }}>
    ...
    <br />
    Plan:
    {value.plan && <pre>{JSON.stringify(value.plan, null, 4)}</pre>}
  </div>
);
...
class FundAllocateForm extends React.Component<TransferAllocateFormProps & MTAllocateState> {
  public render() {
    const maxCashBalance =
      this.props.token === 'DAI' ? this.props.cashBalance.plus(this.props.amount) : this.props.cashBalance;
    return (
      <React.Fragment>
...
class BuyOrderSummary extends React.Component<OrderAllocateFormProps & MTAllocateState> {
  public render() {
    return <OrderSummary {...this.props} deltaDaiBalanceToShow={this.props.deltaDaiBalance.multipliedBy(minusOne)} />;
  }
}

class SellOrderSummary extends React.Component<OrderAllocateFormProps & MTAllocateState> {
  public render() {
    return <OrderSummary {...this.props} deltaDaiBalanceToShow={this.props.deltaDaiBalance} />;
  }
}
      ...
      <div>
        ...
        <div className={styles.dict}>
          <Muted className={styles.dictName}>
            {this.props.targetDaiBalance.gt(this.props.initialDaiBalance)
              ? 'Allocated Dai'
              : this.props.targetDaiBalance.lt(zero)
              ? 'Allocated Dai'
              : 'Dai balance'}
          </Muted>
          <Money
...
class FundOrderSummary extends React.Component<TransferAllocateFormProps & MTAllocateState> {
  public render() {
    return <TransferOrderSummary label="Fund" {...this.props} />;
  }
}

class DrawOrderSummary extends React.Component<TransferAllocateFormProps & MTAllocateState> {
  public render() {
    return <TransferOrderSummary label="Draw" {...this.props} />;
  }
}
...
> {
  public render() {
    ...
    const currentValue = this.props.debt.targetDebt
      ? this.props.debt.targetDebt.toNumber()
      : this.props.debt.debt.toNumber();
    const maxAvailable = Math.min(
      this.props.debt.maxDebt.toNumber(),
      ...
      <div key={debt.name}>
        ...
        <div className={styles.dict}>
          ...
          <div className={styles.dictDescription}>
            {allocationInput(this.handleChange.bind(this), debt.delta, messageContent(message))}
          </div>
        </div>
        <DebtSlider
          min={0}
        ...
        />
        <div className={styles.dict}>
          <Muted className={styles.dictName}>Debt before</Muted>
          <Money
            className={classnames(styles.dictDescription, styles.dictDescriptionMainFont)}
            value={debt.debt}
            token="DAI"
        ...
        <div className={styles.dict}>
          <Muted className={styles.dictName}>Actual debt</Muted>
          <Money
            className={classnames(styles.dictDescription, styles.dictDescriptionMainFont)}
        ...
        <div className={styles.dict}>
          <Muted className={styles.dictName}>Max debt</Muted>
          <Money
            className={classnames(styles.dictDescription, styles.dictDescriptionMainFont)}
            value={debt.maxDebt}
            token="DAI"
        ...
        <div className={styles.dict}>
          <Muted className={styles.dictName}>Liquidation price/ratio</Muted>
          <div className={styles.dictDescription}>
            <Muted>
              <Money value={debt.liquidationPrice} token="DAI" />
            </Muted>
            <span className={classnames(styles.dictDescriptionMainFont, styles.spaceLeft)}>
              {debt.currentCollRatio && <FormatPercent value={debt.currentCollRatio} multiply={true} />}
            </span>
          </div>
        </div>
      </div>
...
> {
  public render() {
    ...
    const maxAvailable = this.props.reversed
      ? Math.max(0, Math.min(this.props.cashBalance.toNumber(), currentValue + this.props.diffDaiBalance.toNumber()))
      : currentValue + this.props.diffDaiBalance.toNumber();
    ...
    return (
      <div>
        <div className={styles.dict}>
          ...
          DAI
          <div className={styles.dictDescription}>
            {allocationInput(this.handleCashChange.bind(this), cashDelta, messageContent(message))}
          </div>
        </div>
        <DebtSlider
          min={0}
...
function allocationInput(
  ...
  return (
    <InputGroup sizer="sm" className={styles.inputGroup} title={errorMessage}>
      <BigNumberInput
        ref={(el: any) => (inputField = (el && (ReactDOM.findDOMNode(el) as HTMLElement)) || undefined)}
        type="text"
        ...
        value={value && formatAmount(value, 'DAI')}
        guide={true}
        placeholderChar={' '}
      ...
      <div key={debt.name} className={classnames(styles.dict, styles.finalizeMarginable)}>
        ...
        <div>
          ...
          <div className={styles.superSmallFont}>
            ...
            <Money value={debt.liquidationPrice} token="DAI" className={classnames(styles.green, styles.spaceLeft)} />
    ...
    return (
      <div>
        <div className={styles.dict}>
          ...
          <span className={classnames(styles.spaceLeft)}>DAI/{this.props.baseToken}</span>
        </div>
        <div className={styles.dict}>
          {getToken(this.props.baseToken).iconColor}
          <span className={styles.dictName}>Buy</span>
          <Money value={this.props.amount} token={this.props.baseToken} className={styles.spaceLeft} />
        </div>
      </div>
    );
  }
...
class TransactionStatus extends React.Component<MTAllocateState> {
  public render() {
    return (
      <div className={styles.dict}>
        <span className={styles.dictName}>Create position</span>
  ...
  public render() {
    return (
      <div className={styles.dict} style={{ marginBottom: 0 }}>
        <Muted className={styles.dictName}>Gas cost</Muted>
        <span className={styles.dictDescription}>
          <GasCost
...
class AllocateButtons extends React.Component<MTAllocateState> {
  public render() {
    ...
    return (
      <React.Fragment>
        <Button size="md" onClick={this.handleAutoAllocate} className={styles.button}>
          Auto allocate
        </Button>
        <Button size="md" disabled={!proceedEnabled} onClick={this.handleProceed} className={styles.button}>
          Proceed
        </Button>
      </React.Fragment>
    );
