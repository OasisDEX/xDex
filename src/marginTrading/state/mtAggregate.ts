    ...
    map(([balanceResult, rawHistories, osmPrices, osmParams, orderbooks]) => {
      const marginables = assetNames.map((token) => {
        return getMarginableCore({
          ...
          ...balanceResult[token],
          allowance: proxy.options.address !== nullAddress ? balanceResult[token].allowance : false,
...
export function createProxyAllowance$(
  ...
  return combineLatest(context$, initializedAccount$, proxyAddress$, onEveryBlock$).pipe(
    switchMap(([context, account, proxyAddress]) =>
      proxyAddress
        ? from(context.tokens[token].contract.methods.allowance(account, proxyAddress).call()).pipe(
