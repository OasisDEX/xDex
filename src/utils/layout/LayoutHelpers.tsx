...
type BorderBoxProps = React.HTMLAttributes<HTMLDivElement> & {
  className?: any;
  children?: any;
  padding?: 'md' | 'sm' | 'none';
};
...
export const BorderBox = ({ className, children, padding, ...props }: BorderBoxProps) => {
  ...
  return (
    <div className={classnames(styles.borderBox, styles[paddingStyle], className)} {...props}>
      {children}
    </div>
  );
};
...
export const Hr = ({ color, className, ...props }: HrProps) => {
  ...
  return <hr className={classnames(styles.hr, styles[colorHr], className)} {...props} />;
};
