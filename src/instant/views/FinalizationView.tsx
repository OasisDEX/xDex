    ...
    return (
      ...
      >
        ...
        <>
          ...
          {progress && progress.kind === ProgressKind.proxyNoAllowancePayWithERC20 && this._tokenAllowanceTx()}
      ...
      <>
        <div className={classnames(styles.details, styles.transaction)} data-test-id="trade-tx">
          ...
          {sai2dai(sellToken) !== sellToken && (
            <TxStatusRow
              icon={getToken(sellToken).iconColor}
              label={
                <TradeData
                  data-test-id="upgrade"
                  theme="reversed"
                  label="Upgrade"
                  value={<Money formatter={formatAmountInstant} value={sellAmount} token={sellToken} />}
                />
              }
              status={
                progress.kind !== ProgressKind.noProxyPayWithETH && <ProgressReport report={this._tradeProgress()} />
              }
            />
          )}
          ...
          <TxStatusRow
            ...
            status={
              ...
              sai2dai(sellToken) === sellToken && <ProgressReport report={this._tradeProgress()} />
            }
          />
          <TxStatusRow
            icon={getToken(buyToken).iconColor}
            label={
              <TradeData
                ...
                value={
                  <Approximate>
                    <Money formatter={formatAmountInstant} value={buyAmount} token={buyToken} />
                  </Approximate>
                }
              />
            }
