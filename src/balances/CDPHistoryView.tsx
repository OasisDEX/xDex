    ...
    return (
      ...
      >
        <Panel style={{ width: '454px' }} className={styles.modalChild}>
          <PanelHeader bordered={true}>Details of {this.props.name} history</PanelHeader>
          <CDPHistoryView {...this.props} />
          <PanelFooter>
            <Button size="md" block={true} onClick={this.props.close}>
              Close
            </Button>
          </PanelFooter>
        </Panel>
            ...
            {this.props.history.map((e, i) => {
              ...
              switch (e.kind) {
                case MTHistoryEventKind.drawDai:
                  ...
                  amount = (
                    <>
                      <FormatCrypto value={dDAIAmount} token="DAI" /> DAI
                    </>
                  );
                  break;
                ...
                case MTHistoryEventKind.fundGem:
                  ...
                  displayName = 'Deposit';
                  break;
                case MTHistoryEventKind.buyMultiple:
                  amount = (
                    <>
                      <FormatFiat value={dAmount} token={e.token} /> {e.token}
                    </>
                  ...
                  break;
                case MTHistoryEventKind.sellMultiple:
                  amount = (
                    <>
                      <FormatFiat value={dAmount} token={e.token} /> {e.token}
                    </>
              ...
              return (
                <tr key={i}>
                  ...
                  <td>
                    <InfoLabel>{formatDateTime(new Date(e.timestamp), true)}</InfoLabel>
                  </td>
                </tr>
              );
            })}
