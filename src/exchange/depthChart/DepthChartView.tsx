...
export class DepthChartView extends React.Component<DepthChartInternalProps> {
  public render() {
    ...
    ;([
      ...
      [data.sellsBefore, sellArea, chartMode === 'sell' ? styles.hidden : styles.sellChartDark],
      ...
      <>
        <PanelHeader bordered={true}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex' }}>
            <Button style={btnStyles} onClick={this.zoomOut} disabled={!data.zoomOutEnabled}>
              <SvgImage image={minusSvg} />
            </Button>
            <Button style={btnStyles} onClick={this.zoomIn} disabled={!data.zoomInEnabled}>
        ...
        <div className={styles.depthChart}>
          ...
          {data.summary && drawSummaryInfoBox(data.summary) && (
            <Legend summary={data.summary} fromCurrency={this.props.base || ''} toCurrency={this.props.quote || ''} />
          )}
        </div>
      </>
    )
...
function drawDotsAndLine(
  ...
  const circleData: [[number, string, ScaleLogarithmic<number, number>]] = [
    extra,
    [chartMode === 'sell' ? 0 : data.summary.currentForSale.amount, styles.dotSell, ySellScale],
...
function drawInfoBoxLine(
  svg: Selection<BaseType, any, null, undefined>,
  circleData: any,
  summary: Summary,
  xPrice: number,
) {
...
const Legend = ({
  ...
  return (
    ...
    >
      <span>
        At {summary.price} {toCurrency} per {fromCurrency}...
      </span>
  ...
  <div className={styles.infoSummaryLine}>
    <div>
      <span className={styles[color]}>
        {new BigNumber(amount).toFixed(2)} {fromCurrency}
      </span>
      <Muted> {describer}</Muted>
    </div>
    <div>
      <span className={styles[color]}>
        {new BigNumber(totalCost).toFixed(2)} {toCurrency}
      </span>
      <Muted> total</Muted>
    </div>
  </div>
